...
### Быстрое обновление через update.sh (рекомендуется)

Используйте скрипт `update.sh` для автоматического обновления:
...
- Исправит проблему "dubious ownership" при необходимости
...
**Примечание:** Скрипт автоматически решает проблему "fatal: detected dubious ownership" при git pull.

### Ручное обновление

Когда нужно обновить код бота вручную, есть несколько способов:
