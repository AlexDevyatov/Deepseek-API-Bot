...
Для развертывания бота на VPS сервере с автозапуском через systemd см. подробное руководство в файле [DEPLOY.md](DEPLOY.md).
...
2. Запустите скрипт развертывания:
   ...
   sudo ./deploy.sh
   ```
3. Настройте токены в `/opt/aibot/tokens.txt`
4. Запустите бота:
   ```bash
...
После первоначального развертывания для обновления бота используйте скрипт `update.sh`:

```bash
cd /opt/aibot  # или путь к вашему проекту
sudo ./update.sh
```

Скрипт автоматически:
- Остановит бота (если запущен)
- Обновит код через Git (если это git репозиторий)
- Обновит зависимости (если изменился requirements.txt)
- Перезапустит бота (если он был запущен)
- Покажет статус и полезные команды

Подробные инструкции, управление сервисом и устранение неполадок описаны в [DEPLOY.md](DEPLOY.md).
...
├── aibot.service         # Systemd service файл для автозапуска
├── deploy.sh              # Скрипт автоматического развертывания
├── update.sh              # Скрипт для обновления бота на сервере
